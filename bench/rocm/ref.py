...
import sageattention._qattn_rocm as qattn
...
parser.add_argument('--pv_accum_dtype', type=str, default='fp32', choices=['fp32', 'fp32+fp32', 'fp32+fp16'])
...
for seq_len in {1024, 2048, 4096}:
    print(f"seqence length is:{seq_len}")
    ...
    t_f32 = torch.zeros((batch, head, seq_len,seq_len), dtype=torch.int32, device="cuda")
    t_f32_dict[seq_len] = t_f32 
    ...
    v = torch.randn(batch, headdim,head,  seq_len, dtype=torch.float16, device="cuda").to(torch.float8_e4m3fnuz)
    sm_scale = 1 / (headdim ** 0.5)
    if not fused_v:
        for i in range(5): kernel(q, k, v, o, q_scale, k_scale, t_f32, 0, _is_causal, _qk_quant_gran, sm_scale, 0)
        ...
        start.record()
        for _ in range(it):
            kernel(q, k, v, o, q_scale, k_scale, t_f32, 0, _is_causal, _qk_quant_gran, sm_scale, 0)
        end.record()

        torch.cuda.synchronize()
        elapsed_ms = start.elapsed_time(end) / it
        print(f"avg time: {elapsed_ms:.3f} ms")
    else:
        for i in range(5): kernel(q, k, v, o, q_scale, k_scale, v_scale, t_f32, 0, _is_causal, _qk_quant_gran, sm_scale, 0)
        torch.cuda.synchronize()
